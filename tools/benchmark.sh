...
prepare() {
    sudo mkdir -p /ramdisk/bench
    sudo mkdir -p /ramdisk/bench/tmp
}
...
run_tests() {
    ...
    do
        ...
        chrt -f 99 /usr/bin/time -f "%e" -o ${output_file} -a ${command_to_run} > /dev/null 2>&1
    ...
    md5sum -b -c <<<"5d76461b53079d20c08eb0b33c46b7cd  /ramdisk/bench/tmp/result"
